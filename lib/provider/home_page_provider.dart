...
import '../service/fake_remider_service.dart';
...
class HomePageProvider extends ChangeNotifier {
  ...
  final List<Home> homeData = dummyData.map((home) {
    return Home.fromJson(home);
  }).toList();
  final List<Home> homeDataList = [];
  ...
  void initState() {
    SchedulerBinding.instance.scheduleFrameCallback((timeStamp) {
      ReminderService().getReminderDataService();
      loadMoreHomeData();
  ...
  void handleItemCreated(int index) {
    ...
    if (loadMore && (index + 1) > homeDataList.length) {
      loadMoreHomeData();
      return;
    }

    if (homeDataList.length == index && index != 0) {
      _isLoadMore = false;
      notifyListeners();
    }
  }
  ...
  void loadMoreHomeData() async {
    ...
    for (var i = 1; i <= _initialDataLength; i++) {
      if (homeData.length > homeDataList.length + 1) {
        homeDataList.addAll([homeData[homeDataList.length + 1]]);
      } else {
        break;
      }
    }
