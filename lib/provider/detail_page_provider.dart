...
class DetailPageProvider extends ChangeNotifier {
  ...
  final List<Home> detailData = dummyData.map((home) {
    return Home.fromJson(home);
  }).toList();
  final List<Home> detailDataList = [];
  final int _initialDataLength = 3;

  late bool _isLoadMore = false;
  bool get isLoadMore => _isLoadMore;

  static String _dateTimeNow =
      DateFormat('dd/MM/yyyy hh:mm:ss a').format(DateTime.now()).toString();
  String get dateTimeNow => _dateTimeNow;
  late Timer timerDateTime;

  void initState() {
    timerDateTime = Timer.periodic(const Duration(seconds: 1), (timer) {
      _dateTimeNow =
          DateFormat('dd/MM/yyyy hh:mm:ss a').format(DateTime.now()).toString();
      notifyListeners();
    });
  ...
  void handleItemCreated(int index) {
    ...
    final bool loadMore = index % _initialDataLength == 0 && index != 0;

    if (loadMore && (index + 1) > detailDataList.length) {
      loadMoreHomeData();
      return;
    }

    if (detailDataList.length == index && index != 0) {
      _isLoadMore = false;
      notifyListeners();
    }
  }

  void loadMoreHomeData() async {
    _isLoadMore = true;
    notifyListeners();
    await Future.delayed(const Duration(milliseconds: 600));

    for (var i = 1; i <= _initialDataLength; i++) {
      if (detailData.length > detailDataList.length + 1) {
        detailDataList.addAll([detailData[detailDataList.length + 1]]);
      } else {
        break;
      }
    }
